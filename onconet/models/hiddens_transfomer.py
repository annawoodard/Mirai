...
class AllImageTransformer(nn.Module):
    def __init__(self, args):
        ...
        self.mask_embedding = torch.nn.Embedding(
            2, args.precomputed_hidden_dim, padding_idx=1
        )
        ...
        self.pred_masked_img_fc = nn.Linear(
            args.hidden_dim, args.precomputed_hidden_dim
        )
    ...
    def mask_input(self, x, view_seq):
        ...
        mask_prob = (
            self.args.mask_prob
            if self.training and self.args.pred_missing_mammos
            else 0
        )
        is_mask = torch.bernoulli(
            self.kept_images_vec.expand([B, N, 1]) * mask_prob
        )  # 0 is not masked, 1 is masked
        # Don't mask out any PAD images
        is_mask = (
            is_mask * (view_seq < MAX_VIEWS).unsqueeze(-1).float()
        )  # sets all not viewed as 0
        is_kept = 1 - is_mask
        ...
        if self.args.also_pred_given_mammos:
            ...
            is_mask = (is_mask >= -1).float() * (view_seq < MAX_VIEWS).unsqueeze(
                -1
            ).float()
        return x, is_mask
    ...
    def get_pred_mask_loss(self, transformer_hidden, x, is_mask):
        ...
        _, _, D_o = x.size()
        hidden_for_mask = torch.masked_select(transformer_hidden, is_mask.byte()).view(
            -1, D_n
        )
    ...
    def forward(self, x, risk_factors=None, batch=None):
        ...
        logit, hidden = self.aggregate_and_classify(
            img_like_hidden, risk_factors=risk_factors
        )
...
class Transformer(nn.Module):
    def __init__(self, args):
        ...
        self.time_embed = torch.nn.Embedding(
            MAX_TIME + 1, EMBEDDING_DIM // 3, padding_idx=-1
        )
        self.view_embed = torch.nn.Embedding(
            MAX_VIEWS + 1, EMBEDDING_DIM // 3, padding_idx=-1
        )
        self.side_embed = torch.nn.Embedding(
            MAX_SIDES + 1, EMBEDDING_DIM // 3, padding_idx=-1
        )
