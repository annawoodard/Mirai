...
class AttentionPool2d(AbstractPool):
    def __init__(self, args, num_chan):
        ...
        self.activation_value_conv = nn.Conv2d(
            self.C, self.C, kernel_size=1, bias=False
        )
    ...
    def compute_attention(self, x):
        ...
        if num_dim == 4:
            activation_key = activation_key.permute(
                [0, 2, 3, 1]
            ).contiguous()  # (B, W, H, C)
        else:
            assert num_dim == 5
    ...
    def forward(self, x):
        ...
        if num_dim == 4:
            activation_value = activation_value.permute(
                [0, 2, 3, 1]
            ).contiguous()  # (B, W, H, C)
        else:
            assert num_dim == 5
