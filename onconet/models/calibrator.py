...
class MiraiCalibrator:
    ...
    def __init__(
        self,
        base_estimator_slope,
        base_estimator_offset,
        calibrator_slope,
        calibrator_offset,
    ):
...
def from_sk_calibrator(sk_calibrator: "sklearn.calibration.CalibratedClassifierCV"):
    ...
    assert len(sk_calibrator.calibrated_classifiers_) == 1, (
        "Only one calibrated classifier is supported."
    )
    assert len(sk_calibrator.calibrated_classifiers_[0].calibrators_) == 1, (
        "Only one calibrator is supported."
    )
    calibrator = sk_calibrator.calibrated_classifiers_[0].calibrators_[0]
    return MiraiCalibrator(
        base_estimator.coef_[0],
        base_estimator.intercept_[0],
        calibrator.a_,
        calibrator.b_,
    )
