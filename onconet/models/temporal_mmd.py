...
from onconet.models.mmd import gaussian_kernel
...
class TemporalMMDDiscriminator(nn.Module):
    ...
    def moving_mean(self, distances, x_ages, y_ages):
        ...
        moving_average = torch.sum(distances * discount_matrix) / torch.sum(
            discount_matrix
        )
    ...
    def forward(self, x, y):
        ...
        cost = self.moving_mean(
            gaussian_kernel(x_with_mem, x_with_mem, self.sigmas), x_ages, x_ages
        )
        cost += self.moving_mean(
            gaussian_kernel(y_with_mem, y_with_mem, self.sigmas), y_ages, y_ages
        )
        cost -= 2 * self.moving_mean(
            gaussian_kernel(x_with_mem, y_with_mem, self.sigmas), x_ages, y_ages
        )
