...
def plot_losses(args):
    ...
    for key in losses:
        ...
        try:
            ...
            print(ERR_PLOT_STR.format(key, save_path, e))
...
def plot_pred_stats(golds_percent, pred_percent, save_path):
    ...
    rec1 = ax.bar(
        ind - width / 2, pred_percent.values(), width, align="center", label="Model"
    )
    rec2 = ax.bar(
        ind + width / 2, golds_percent.values(), width, align="center", label="Human"
    )
    ax.legend()
...
def plot_roc_curve(fr, tr, class_name, save_path):
    ...
    plt.plot(
        ...
        color="darkorange",
        lw=lw,
        label="{} ROC curve (area = {:.2f})".format(class_name, roc_auc),
    )
