...
def get_model_loss(logit, y, batch, args):
    if args.survival_analysis_setup:
        if args.pred_both_sides:
            ...
            for side in "l", "r":
                y_seq, y_mask = (
                    batch["y_seq_{}".format(side)],
                    batch["y_mask_{}".format(side)],
                )
...
def get_region_loss(activ_dict, logit, batch, train_model, args):
    ...
    region_annotation_mask = get_annotation_mask(
        activ, batch, volatile, args
    )  # [B, (T), H, W]

    any_region_defined = (
        has_region_defined.max().data.item() == 1
        and torch.sum(region_annotation_mask).data.item() > 0
    )
    ...
    if num_dim == 4:
        ...
        sample_mask = (
            has_region_defined.expand([H * W, B, T])
            .permute([1, 2, 0])
            .contiguous()
            .view([B, -1])
            .float()
    ...
    flat_region_annotation_mask = flat_region_annotation_mask / (
        torch.sum(flat_region_annotation_mask, dim=1).unsqueeze(-1) + epsilon
    )

    if region_loss_type == "pred_region":
        region_preds = F.sigmoid(region_logits)
        target_probs = (
            region_preds * (region_annotation_mask == 1).float()
            + (1 - region_preds) * (region_annotation_mask == 0).float()
        )
        focal_loss_weighting = (
            (1 - target_probs) ** args.region_annotation_focal_loss_lambda
            if args.region_annotation_focal_loss_lambda > 0
            else 1
        )
        masked_focal_loss_weighting = (
            focal_loss_weighting * sample_mask.view_as(target_probs).float()
        )
        focal_loss = F.binary_cross_entropy_with_logits(
            region_logits,
            region_annotation_mask.float(),
            weight=masked_focal_loss_weighting.detach(),
            size_average=False,
        )
...
def model_step(x, y, risk_factors, batch, models, optimizers, train_model, args):
    ...
    if args.use_risk_factors:
        logit, hidden, activ_dict = models["model"](
    ...
    if args.pred_both_sides:
        logit, _ = torch.max(
            torch.cat([logit["l"].unsqueeze(-1), logit["r"].unsqueeze(-1)], dim=-1),
            dim=-1,
        )
    ...
    if args.pred_missing_mammos:
        pred_masked_mammo_loss = (
            activ_dict["pred_masked_mammo_loss"].mean()
            if args.data_parallel
            else activ_dict["pred_masked_mammo_loss"]
        )
    ...
    if args.use_adv:
        ...
        train_adv = (
            not args.use_mmd_adv
            and train_model
            and (args.step_indx != 0 or not args.train_adv_seperate)
        )
        if train_adv:
            num_adv_steps = 0
            adv_loss = adv_step(
                hidden, logit, batch, models, optimizers, train_model, args
            )
            while (
                adv_loss > args.device_entropy
                and num_adv_steps < args.num_adv_steps
                and not args.train_adv_seperate
            ):
                num_adv_steps += 1
                adv_loss = adv_step(
                    hidden, logit, batch, models, optimizers, train_model, args
        ...
        if args.turn_off_model_train or (
            train_model and args.step_indx != 0 and args.train_adv_seperate
        ):
            loss *= 0
            reg_loss *= 0
...
def get_cross_entropy_adv_loss(models, hidden, logit, batch, args):
    ...
    device, device_known, y = (
        batch["device"],
        batch["device_is_known"].float(),
        batch["y"],
    )
    ...
    adv_loss_per_sample = (
        F.cross_entropy(device_logit, device, reduce=False) * device_known
    )
...
def get_mmd_loss(models, hidden, logit, batch, args):
    ...
    img_hidden = (
        hidden[:, : args.img_only_dim]
        if args.use_risk_factors and not args.use_precomputed_hiddens
        else hidden
    )
    img_only_dim = img_hidden.size()[-1]
    ...
    source_pos_hidden = torch.masked_select(
        img_hidden, (is_source_device * is_pos).unsqueeze(-1)
    ).view(-1, img_only_dim)
    source_neg_hidden = torch.masked_select(
        img_hidden, (is_source_device * is_neg).unsqueeze(-1)
    ).view(-1, img_only_dim)

    is_target_device = (device == 2) * (device_known == 1)
    target_pos_hidden = torch.masked_select(
        img_hidden, (is_target_device * is_pos).unsqueeze(-1)
    ).view(-1, img_only_dim)
    target_neg_hidden = torch.masked_select(
        img_hidden, (is_target_device * is_neg).unsqueeze(-1)
    ).view(-1, img_only_dim)

    pos_hidden = torch.masked_select(img_hidden, is_pos.unsqueeze(-1)).view(
        -1, img_only_dim
    )
    neg_hidden = torch.masked_select(img_hidden, is_neg.unsqueeze(-1)).view(
        -1, img_only_dim
    )
    ...
    if (
        args.add_repulsive_mmd
        and pos_hidden.nelement() > 0
        and neg_hidden.nelement() > 0
    ):
