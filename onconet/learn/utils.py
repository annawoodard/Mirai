...
def compute_eval_metrics(
    ...
    if args.survival_analysis_setup:
        return compute_eval_metrics_survival(
            args,
            loss,
            golds,
            preds,
        ...
        return compute_eval_metrics_classifcation(
            args,
            loss,
            golds,
            preds,
...
def compute_eval_metrics_survival(
    args,
    loss,
    golds,
    preds,
    ...
    metrics, sample_sizes = compute_auc_metrics_given_curve(
        probs,
        censor_times,
        golds,
        years,
    ...
    for followup in range(args.max_followup):
        for allow_all_years in [True]:
            ...
            metric_key = (
                min_followup_if_neg
                if allow_all_years
                else "year_filtered_{}".format(min_followup_if_neg)
            )
...
def compute_auc_metrics_given_curve(
    probs, censor_times, golds, years, max_followup, censor_distribution
):
    metrics = {}
    sample_sizes = {}
    for followup in range(max_followup):
        ...
        auc, golds_for_eval = compute_auc_x_year_auc(
...
def compute_eval_metrics_classifcation(
    args,
    loss,
    golds,
    preds,
    ...
    stats_dict["{}_confusion_matrix".format(key_prefix)].append(
    ...
    if (
        (args.num_classes == 2 or args.predict_birads)
        and args.threshold is not None
        and "1year" in args.dataset
    ):
        try:
            (fnr, fnr_ci), (tpr, tpr_ci), (tnr, tnr_ci) = stats.get_rates_intervals(
                probs,
                golds,
                args.threshold,
