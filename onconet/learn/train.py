...
from onconet.learn.utils import (
    cluster_results_by_exam,
    ignore_None_collate,
    init_metrics_dictionary,
    get_train_and_dev_dataset_loaders,
    compute_eval_metrics,
    get_human_preds,
)
from onconet.learn.step import model_step
import warnings
...
def get_train_variables(args, model):
    ...
    if args.use_adv:
        if args.use_mmd_adv:
            adv_name = (
                "temporal_mmd_discriminator"
                if args.use_temporal_mmd
                else "mmd_discriminator"
            )
            ...
            if args.add_repulsive_mmd:
                models["repel_adv"] = model_factory.get_model_by_name(
                    adv_name, False, args
                )
        else:
...
def train_model(train_data, dev_data, model, args):
    ...
    train_data_loader, dev_data_loader = get_train_and_dev_dataset_loaders(
        args, train_data, dev_data, batch_size
    )
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for mode, data_loader in [
            ...
            loss, golds, preds, probs, exams, reg_loss, censor_times, adv_loss = (
                run_epoch(
                    data_loader,
            ...
            if mode == "Dev" and "mammo_1year" in args.dataset:
                ...
                log_statement = "{} fnr: {:.3f} tnr: {:.3f} tpr: {:.3f}".format(
                    log_statement, fnr, tnr, tpr
                )
        ...
        best_func, arg_best = (
            (min, np.argmin) if tuning_key == "dev_loss" else (max, np.argmax)
        )
        ...
        if num_epoch_sans_improvement >= args.patience or (
            no_tuning_on_dev
            and num_epoch_since_reducing_lr >= args.lr_reduction_interval
        ):
            ...
            if not args.turn_off_model_reset:
                ...
                for name in optimizers:
                    ...
                    optimizers[name] = state_keeper.load_optimizer(
...
def compute_threshold_and_dev_stats(dev_data, models, args):
    ...
    if (
        ("detection" in args.dataset or "risk" in args.dataset)
        and "1year" in args.dataset
        and not args.survival_analysis_setup
    ):
        ...
        print(
            " Dev Threshold: {:.8f} ({:.8f} - {:.8f})".format(threshold, th_lb, th_ub)
        )
    else:
        args.threshold = None
...
def run_epoch(data_loader, train_model, truncate_epoch, models, optimizers, args):
    ...
    if truncate_epoch:
        max_batches = (
            args.max_batches_per_train_epoch
            if train_model
            else args.max_batches_per_dev_epoch
        )
    ...
    for batch in data_iter:
        ...
        step_results = model_step(
            x, y, risk_factors, batch, models, optimizers, train_model, args
        )
        ...
        batch_reg_loss += (
            reg_loss
            if isinstance(reg_loss, int) or isinstance(reg_loss, float)
            else reg_loss.cpu().data.item()
        )
        batch_adv_loss += (
            adv_loss
            if isinstance(adv_loss, int) or isinstance(adv_loss, float)
            else adv_loss.cpu().data.item()
        )
        if train_model:
    ...
    if args.cluster_exams:
        ...
        if "risk" in args.dataset or "detection" in args.dataset:
            aggr = "max"
        gold_set, preds, probs, exam_set = cluster_results_by_exam(
            golds, preds, probs, exams, aggr=aggr
        )
        if args.survival_analysis_setup:
            ...
            for exam in exam_set:
                censor_time_set.append(
                    exam_to_censor_details[exam][1]
                    if 1 in exam_to_censor_details[exam]
                    else exam_to_censor_details[exam][0]
                )
...
def get_hiddens(dataset, models, args):
    ...
    for batch in tqdm(data_loader):
        ...
        with torch.no_grad():
            ...
            step_results = model_step(
                x, y, risk_factors, batch, models, None, False, args
            )
            _, _, _, _, _, _, batch_hidden, _ = step_results
            batch_hidden = (
                batch_hidden[:, : args.img_only_dim]
