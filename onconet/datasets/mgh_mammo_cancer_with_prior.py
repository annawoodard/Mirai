                ...
                for view in all_views:
                    ...
                    for prior_year, prior in prior_with_view:
                        years_and_label = "cur_year:{},prior_year:{},label:{}".format(
                            year, prior_year, exam["{} y".format(side)]
                        )

                        if split_group == "train":
                            if not (
                                year in self.args.train_years
                                and prior_year in self.args.train_years
                            ):
                                continue
...
class MGH_Mammo_5Year_Risk_With_Prior(Abstract_MGH_Mammo_Cancer_With_Prior_Dataset):
    def __init__(self, args, transformer, split_group):
        self.years = 5
        super(MGH_Mammo_5Year_Risk_With_Prior, self).__init__(
...
class MGH_Mammo_1Year_Risk_With_Prior(Abstract_MGH_Mammo_Cancer_With_Prior_Dataset):
    def __init__(self, args, transformer, split_group):
        self.years = 1
        super(MGH_Mammo_1Year_Risk_With_Prior, self).__init__(
            args, transformer, split_group
        )
...
class MGH_Mammo_1Year_Detection_With_Prior(
    Abstract_MGH_Mammo_Cancer_With_Prior_Dataset
):
    def __init__(self, args, transformer, split_group):
        self.years = 1
        super(MGH_Mammo_1Year_Detection_With_Prior, self).__init__(
            args, transformer, split_group
        )
