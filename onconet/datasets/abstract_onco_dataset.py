...
from onconet.utils.region_annotation import (
    ...
    get_region_annotation_for_sample,
)
...
class Abstract_Onco_Dataset(data.Dataset):
        ...
        try:
            ...
            if "all" not in self.args.allowed_devices:
                self.dataset = [
                    ...
                    if (
                        d["device_name"]
                        if isinstance(d["device_name"], str)
                        else d["device_name"][0]
                    )
                    in self.args.allowed_devices
                ]
        except:
    ...
    def get_vector_item(self, index):
        try:
            ...
            def get_hidden(path):
                ...
                return (
                    self.path_to_hidden_dict[path]
                    if path in self.path_to_hidden_dict
                    and not self.args.zero_out_hiddens
                    else zero_vec
                )
                ...
                if self.args.use_region_annotation:
                    for img_index, path in enumerate(sample["paths"]):
                        if img_index == len(additionals):
                            additionals.append(
                                {"region_annotation": region_annotation[img_index]}
                            )
                        else:
                            additionals[img_index]["region_annotation"] = (
                                region_annotation[img_index]
                            )
                x = self.image_loader.get_images(sample["paths"], additionals)
            else:
                additional = (
                    {} if sample["additional"] is None else sample["additional"]
                )
                if self.args.use_region_annotation:
            ...
            if self.args.use_region_annotation:
                if self.args.multi_image:
                    for coord in region_annotation[0]:
                        annotation_list = (
                            lambda coord=coord, region_annotation=region_annotation: [
    ...
    def add_risk_factors_to_dataset(self):
        for sample in self.dataset:
            sample["risk_factors"] = (
                self.risk_factor_vectorizer.get_risk_factors_for_sample(sample)
            )
    ...
    def add_device_to_dataset(self):
        ...
        device_dist = Counter(
            [d["device"] if single_image else d["device"][-1] for d in self.dataset]
        )
    ...
    def build_path_to_device_map(self):
        ...
        for mrn_row in json.load(
            ...
            for exam in mrn_row["accessions"]:
                ...
                for file, device, view in zip(
                    exam["files"], exam["manufacturer_models"], exam["views"]
                ):
                    device_name = (
                        "{} {}".format(device, "C-View") if "C-View" in view else device
                    )
        ...
        def get_view(view_name):
            ...
            image_paths = (
                lambda image_paths, source_dir: [
                    source_dir + path for _, path in image_paths_w_view
                ]
            )(image_paths_w_view, source_dir)
            return image_paths
