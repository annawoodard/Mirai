...
from onconet.utils.region_annotation import (
    ...
    get_region_annotation_for_path,
)
...
SUMMARY_MSG = (
    "Contructed {} {} dataset with {} records, and the following class balance \n {}"
)
...
class MGH_Mammo_Patch_Risk_Dataset(Abstract_Onco_Dataset):
    def __init__(self, args, transformer, split_group):
        ...
        super(MGH_Mammo_Patch_Risk_Dataset, self).__init__(
    ...
    def _add_sample(
        self, dataset, path, label, additional, class_balance, year, accession
    ):
            ...
            for exam in exams:
                ...
                if self.check_label(exam):
                    ...
                    for image_path in exam["files"]:
                        bboxes = []
                        if label == 1:
                            region_annotation = get_region_annotation_for_path(
                                image_path, self.region_annotations
                            )
                            ...
                            min_x, max_x = (
                                region_annotation["region_bottom_left_x"],
                                region_annotation["region_bottom_right_x"],
                            )
                            width = max_x - min_x

                            center_x = (max_x + min_x) / 2
                            min_y, max_y = (
                                region_annotation["region_top_left_y"],
                                region_annotation["region_bottom_left_y"],
                            )
                        ...
                        label = 0
                        additional = {"height": 1, "width": 1, "all_overlays": bboxes}
