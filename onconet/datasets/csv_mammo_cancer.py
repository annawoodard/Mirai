...
class CSV_Mammo_Cancer_Survival_All_Images_Dataset(Abstract_Onco_Dataset):
    ...
    def create_dataset(self, split_group, img_dir):
        ...
        for _row in self.metadata_json:
            row = {
                k.encode("ascii", "ignore").decode(): v.encode(
                    "ascii", "ignore"
                ).decode()
                for k, v in _row.items()
            }
            patient_id, exam_id, split = (
                row["patient_id"],
                row["exam_id"],
                row["split_group"],
            )
    ...
    def check_label(self, row):
        valid_pos = (
            row["years_to_cancer"] < self.args.max_followup
            and row["years_to_cancer"] >= 0
        )
