...
try:
    ...
    print(
        "skimage not found, install with 'pip install scikit-image' to use MNIST dataset"
    )
...
class MNIST_Dataset(data.Dataset):
    ...
    def __init__(self, args, transformers, split_group):
        ...
        if self.split_group == "train":
            self.dataset = datasets.MNIST("mnist", train=True, download=True)
        else:
...
class MNIST_Binary_Full_Future_Dataset(MNIST_Dataset):
    ...
    def __init__(self, args, transformers, split_group):
        ...
        super(MNIST_Binary_Full_Future_Dataset, self).__init__(
            args, transformers, split_group
        )
    ...
    def __getitem__(self, index):
        ...
        item["y"] = self.class_mapping[item["y"].item()]
        item["y_seq"] = (
            torch.ones(self.args.max_followup)
            if item["y"]
            else torch.zeros(self.args.max_followup)
        )
...
class MNIST_Biased_Dataset(data.Dataset):
    ...
    def __init__(self, args, transformers, split_group):
        ...
        if self.split_group == "train":
            ...
            mnist_train = [
                self.force_rbg(mnist_train[i]) for i in range(len(mnist_train))
            ]
            self.dataset = [
                self.bias(mnist_train[i])
                for i in range(len(mnist_train) - self.NUM_UNBAISED_TRAIN)
            ]
            self.ref_dataset = [
                mnist_train[i]
                for i in range(
                    len(mnist_train) - self.NUM_UNBAISED_TRAIN, len(mnist_train)
                )
            ]
    ...
    def __getitem__(self, index):
        ...
        item = {"x": x, "y": y, "target_x": target_x, "target_y": target_y}
...
class MNIST_Location(MNIST_Noise):
    ...
    def make_blob(self, radius):
        ...
        rv = multivariate_normal(
            mean=[(radius - 1) / 2, (radius - 1) / 2],
            cov=[[2 * radius, 0], [0, 2 * radius]],
        )
        probs = rv.pdf(points)
