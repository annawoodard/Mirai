...
def build_path_to_hidden_dict(args):
    ...
    print(
        "Built path to hidden dict with {} paths, of dim: {}".format(
            len(path_to_hidden), hiddens[0].shape[0]
        )
    )
...
def get_dataset(args, transformers, test_transformers):
    ...
    if args.use_precomputed_hiddens:
        ...
        if args.force_input_dim:
            ...
            path_to_hidden_dict = (
                lambda input_dim, path_to_hidden_dict: {
                    k: v[:input_dim] for k, v in path_to_hidden_dict.items()
                }
            )(args.input_dim, path_to_hidden_dict)
