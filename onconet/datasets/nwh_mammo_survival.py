...
class NWH_Mammo_Cancer_Survival(Abstract_Onco_Dataset):
    ...
    def check_label(self, row):
        real_screen_date = date_from_str(
            row["images"][0]["dicom_metadata"]["StudyDate"]
        )
        screen_date_valid = date_from_str(row["bl.date"]) == real_screen_date

        has_followup = (
            date_from_str(row["last.appt.date"]) - real_screen_date
        ).days // 365 > 0
        has_cancer = (
            min(date_from_str(row["date.DCIS"]), date_from_str(row["date.BC"]))
            - real_screen_date
        ).days // 365 < self.args.max_followup
