...
class NWH_Mammo_Cancer_Survival_All_Images_Both_Sides_Dataset(
    NWH_Mammo_Cancer_Survival
):
    """
    ...
    def create_dataset(self, split_group, img_dir):
        ...
        for mrn_row in tqdm.tqdm(self.metadata_json):
            ...
            if len(all_images_so_far) < self.args.min_num_images:
                continue
            if (
                not all([img in self.path_to_hidden_dict for img in all_images_so_far])
                and self.args.use_precomputed_hiddens
            ):
                continue
    ...
    def image_paths_by_views(self, images):
        left_ccs = [
            m["png_path"]
            for m in images
            if m["dicom_metadata"]["SeriesDescription"] == "L CC"
        ]
        right_ccs = [
            m["png_path"]
            for m in images
            if m["dicom_metadata"]["SeriesDescription"] == "R CC"
        ]
        left_mlos = [
            m["png_path"]
            for m in images
            if m["dicom_metadata"]["SeriesDescription"] == "L MLO"
        ]
        right_mlos = [
            m["png_path"]
            for m in images
            if m["dicom_metadata"]["SeriesDescription"] == "R MLO"
        ]
