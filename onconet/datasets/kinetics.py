        ...
        try:
            ...
            if len(video) <= self.NUM_FRAMES * self.STRIDE:
                ...
                frame_start = np.random.randint(
                    len(video) - self.NUM_FRAMES * self.STRIDE
                )

            # Select frames in clip and loop around to start if necessary
            x = [
                video.get_data((frame_start + i) % len(video))
                for i in range(0, self.NUM_FRAMES, self.STRIDE)
            ]

            for transformer in self.transformers:
                x = transformer(x)

            item = {"x": x, "y": self.label_map[sample["label"]]}
