...
class Florida_Cancer(Abstract_Florida_Cancer_Dataset):
    def __init__(self, args, transformer, split_group):
        ...
        * The dataset will contain the image path, and the label
        """
        super(Florida_Cancer, self).__init__(args, transformer, split_group)
    ...
    def create_dataset(self, split_group, img_dir):
        ...
        for exam in tqdm.tqdm(self.metadata_json):
            ...
            def fits_criteria(exam):
                group_valid = (
                    "split_group" in exam and exam["split_group"] == split_group
                )
            ...
            for img in exam["imgs"]:
                ...
                dataset.append({"path": img["file_path"], "y": label, "additional": {}})
...
class Florida_Cancer_Multi_Breast(Abstract_Florida_Cancer_Dataset):
    def __init__(self, args, transformer, split_group):
        ...
        * The dataset will contain the image path, and the label
        """
        super(Florida_Cancer_Multi_Breast, self).__init__(
        ...
        for exam in tqdm.tqdm(self.metadata_json):
            ...
            for img in exam["imgs"]:
                ...
                if "CC" in img["file_path"]:
                    ...
                    raise Exception(
                        UNRECOGNIZED_BREAST_ERR.format("breast side", img["file_path"])
                    )
                ...
                if "RIGHT" in img["file_path"]:
                    ...
                    raise Exception(
                        UNRECOGNIZED_BREAST_ERR.format("breast view", img["file_path"])
                    )
            ...
            for view in [ccs, mlos]:
                ...
                dataset.append({"paths": img_paths, "y": label, "additionals": []})
...
class Florida_Cancer_Multiview(Abstract_Florida_Cancer_Dataset):
    def __init__(self, args, transformer, split_group):
        ...
        * The dataset will contain the image path, and the label
        """
        super(Florida_Cancer_Multiview, self).__init__(args, transformer, split_group)
    ...
    def create_dataset(self, split_group, img_dir):
        ...
        for exam in tqdm.tqdm(self.metadata_json):
            ...
            for side in [left_breast, right_breast]:
                ...
                dataset.append({"paths": img_paths, "y": side_label, "additionals": []})

                if side_label not in class_balance:
