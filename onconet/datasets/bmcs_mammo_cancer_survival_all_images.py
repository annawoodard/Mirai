...
class BMCS_Mammo_Cancer_Survival_All_Images_Dataset_All_Device(Abstract_Onco_Dataset):
            ...
            for exam in exams:
                ...
                if self.check_label(exam):
                    ...
                    dataset.append(
                        {
                            "paths": pad_to_length(
                                all_images, "<PAD>", self.args.num_images
                            ),
                            "y": y,
                            "y_mask": y_mask,
                            "y_seq": y_seq,
                            ...
                            "time_seq": pad_to_length(
                                time_stamps, MAX_TIME, self.args.num_images
                            ),
                            "view_seq": pad_to_length(
                                all_views, MAX_VIEWS, self.args.num_images
                            ),
                            "side_seq": pad_to_length(
                                all_sides, MAX_SIDES, self.args.num_images
                            ),
                            "additionals": [],
                        }
                    )
    ...
    def image_paths_by_views(self, exam):
        def get_view(view_name):
            ...
            image_paths = (
                [image_paths_w_view[-1][-1]] if len(image_paths_w_view) > 0 else []
            )
            return image_paths
    ...
    def is_valid_device(self, row):
        return all(["2000D" not in man for man in row["manufacturer_model"]])
    ...
    def get_label(self, row):
        ...
        if y:
            ...
            time_at_event = int(
                min(row["years_to_last_followup"], self.args.max_followup) - 1
            )
...
class BMCS_Mammo_Cancer_Survival_All_Images_Dataset_GE_All_Device(
    BMCS_Mammo_Cancer_Survival_All_Images_Dataset_All_Device
):
    def is_valid_device(self, row):
        return all(
            [
                "2000D" not in man and "Selenia" not in man
                for man in row["manufacturer_model"]
            ]
        )


@RegisterDataset("bmcs_ge_essential_full_future")
class BMCS_Mammo_Cancer_Survival_All_Images_Dataset_GE_Essential_Device(
    BMCS_Mammo_Cancer_Survival_All_Images_Dataset_All_Device
):
    def is_valid_device(self, row):
...
class BMCS_Mammo_Cancer_Survival_All_Images_Dataset_Hologic_Device(
    BMCS_Mammo_Cancer_Survival_All_Images_Dataset_All_Device
):
    def is_valid_device(self, row):
