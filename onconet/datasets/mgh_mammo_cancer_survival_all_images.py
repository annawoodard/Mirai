...
from onconet.datasets.mgh_mammo_cancer_survival import (
    MGH_Mammo_Cancer_Survival_Dataset,
)
import tqdm
import numpy as np
...
BIRADS_TO_PROB = {
    ...
    "1-Negative": 0.0,
    "2-Benign": 0.0,
    "0-Additional imaging needed": 1.0,
    "3-Probably benign": 1.0,
    "4-Suspicious": 1.0,
    "5-Highly suspicious": 1.0,
    "6-Known malignancy": 1.0,
}
MAX_VIEWS = 2
MAX_SIDES = 2
MAX_TIME = 10
            ...
            for side in ["L", "R"]:
                ...
                for i, exam in enumerate(exams):
                    ...
                    if self.check_label(exam):
                        left_ccs, left_mlos, right_ccs, right_mlos = (
                            self.image_paths_by_views(exam)
                        )
                        if side == "L":
                        ...
                        time_stamps_so_far = [
                            t + year_delta for t in time_stamps_so_far
                        ]
                        # Update with new data
                        all_images_so_far.extend(
                            right_ccs + right_mlos + left_ccs + left_mlos
                        )
                        all_views_so_far.extend(
                            [0] * len(right_ccs)
                            + [1] * len(right_mlos)
                            + [0] * len(left_ccs)
                            + [1] * len(left_mlos)
                        )
                        all_sides_so_far.extend(
                            [0] * len(right_ccs)
                            + [0] * len(right_mlos)
                            + [1] * len(left_ccs)
                            + [1] * len(left_mlos)
                        )
                        time_stamps_so_far.extend(
                            [0] * len(right_ccs + right_mlos + left_ccs + left_mlos)
                        )
                        ...
                        if (
                            time_at_event < 0
                        ):  # Case of negative breast with 0 yr follow but pos other breast
                            continue

                        dist_key = "neg"
                        if (
                            y_seq[0] == 1
                            and self.args.shift_class_bal_towards_imediate_cancers
                        ):
                            dist_key = "pos:1"
                        elif y:
                            dist_key = "pos:any"
                        ...
                        dataset.append(
                            {
                                "paths": pad_to_length(
                                    all_images_so_far, "<PAD>", self.args.num_images
                                ),
                                "y": y,
                                "y_mask": y_mask,
                                "y_seq": y_seq,
                                ...
                                "dist_key": dist_key,
                                "ssn": ssn,
                                "time_seq": pad_to_length(
                                    time_stamps_so_far, MAX_TIME, self.args.num_images
                                ),
                                "view_seq": pad_to_length(
                                    all_views_so_far, MAX_VIEWS, self.args.num_images
                                ),
                                "side_seq": pad_to_length(
                                    all_sides_so_far, MAX_SIDES, self.args.num_images
                                ),
                            }
                        )
        return dataset
...
class MGH_Mammo_Cancer_Survival_All_Images_Both_Sides_Dataset(
    MGH_Mammo_Cancer_Survival_Dataset
):
    """
    Working dataset for suvival analysis. Note, does not support invasive cancer yet.
