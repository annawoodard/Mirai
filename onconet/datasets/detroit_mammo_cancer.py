...
POSITIVE_PATIENT_AND_EXAM_MSG = (
    "Number of positive exams: {}, number of positive patients: {}"
)
...
class Abstract_Detroit_Mammo_Cancer_Dataset(Abstract_Onco_Dataset):
    def create_dataset(self, split_group, img_dir):
        ...
        print(
            POSITIVE_PATIENT_AND_EXAM_MSG.format(
                len(positive_exams), len(positive_patients)
            )
        )
        return dataset
...
class Detroit_Mammo_2Year_Detection(Abstract_Detroit_Mammo_Cancer_Dataset):
    def __init__(self, args, transformer, split_group):
        self.years = 2
        super(Detroit_Mammo_2Year_Detection, self).__init__(
...
class Detroit_Mammo_1Year_Detection(Abstract_Detroit_Mammo_Cancer_Dataset):
    def __init__(self, args, transformer, split_group):
        self.years = 1
        super(Detroit_Mammo_1Year_Detection, self).__init__(
            args, transformer, split_group
        )
