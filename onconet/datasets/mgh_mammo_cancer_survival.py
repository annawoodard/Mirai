...
class MGH_Mammo_Cancer_Survival_Dataset(Abstract_Onco_Dataset):
        ...
        for mrn_row in tqdm.tqdm(self.metadata_json):
            ...
            for exam in exams:
                ...
                if self.check_label(exam):
                    ...
                    for image_paths, label in [
                        (left_ccs + left_mlos, left_label),
                        (right_ccs + right_mlos, right_label),
                    ]:
    ...
    def check_label(self, row):
        ...
        valid_history = (
            True
            if not self.task == "Risk" or self.args.use_permissive_cohort
            else row["years_since_cancer"] > 0
        )
        no_cancer_now = (
            True
            if not self.task == "Risk" or self.args.use_permissive_cohort
            else row["years_to_cancer"] > 0
        )
    ...
    def get_label(self, row, side="Any"):
        ...
        if y:
            ...
            if self.args.mask_mechanism == "linear":
                ...
                y_seq = np.array(
                    [
                        (i + 1) * year_hazard if v < 1.0 else v
                        for i, v in enumerate(list(y_seq))
                    ]
                )
