...
EXAM_TYPES = ["benigns", "cancers", "normals"]  # don't include benign_without_callback
...
class Florida_Patch_Abnormalities(Abstract_Onco_Dataset):
    def __init__(self, args, transformer, split_group):
        ...
        * The dataset will contain the image path, the label and additional data in a dictionary
        ...
        """
        super(Florida_Patch_Abnormalities, self).__init__(
    ...
    def check_label(self, overlay):
        ...
        lesion_valid = (
            "leison_type" in overlay and overlay["leison_type"] in LESION_TYPES
        )
        pathology_valid = (
            "pathology" in overlay and overlay["pathology"] in PATHOLOGY_TYPES
        )
    ...
    def get_label(self, overlay):
        if overlay["pathology"] == "BENIGN":
            if overlay["leison_type"] == "CALCIFICATION":
                ...
                raise (
                    Exception(
                        OVERLAY_LABEL_ERR.format(
                            overlay["pathology"], overlay["leison_type"]
                        )
                    )
                )
    ...
    def _add_sample(self, dataset, img, label, additional, class_balance):
        ...
        dataset.append({"path": img["file_path"], "y": label, "additional": additional})
