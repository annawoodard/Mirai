...
def parse_args():
    ...
    # setup
    parser.add_argument(
        "--run_prefix", default="snapshot", help="what to name this type of model run"
    )
    parser.add_argument(
        "--train",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Predefine threshold to compute test rates by it",
    )
    parser.add_argument(
        "--ensemble_paths",
        nargs="*",
        default=[],
        help="The list of snapshot paths to build the ensemble from.",
    )
    parser.add_argument(
        "--train_years",
        nargs="*",
        ...
        help="The list of years to draw training data for mammo risk datasets.",
    )
    parser.add_argument(
        "--dev_years",
        nargs="*",
        type=int,
        default=[2016, 2015, 2014, 2013, 2012, 2011, 2010, 2009],
        help="The list of years to draw dev data for mammo risk datasets.",
    )
    parser.add_argument(
        "--test_years",
        nargs="*",
        type=int,
        default=[2016, 2015, 2014, 2013, 2012, 2011, 2010, 2009],
        help="The list of years to draw test data for mammo risk datasets.",
    )
    parser.add_argument(
        "--predict_birads",
        action="store_true",
        default=False,
        help="Wether to predict birads label for negative mammos in risk dataset objects. Note, preds, probs, and labels converted to binary (cancer vs negative) after prediction for logging purposes",
    )
    parser.add_argument(
        "--predict_birads_lambda",
        type=float,
        default=0,
        help="Lambda to weight birads prediction loss",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether or not only consier invasive as pos (and dcis as neg)",
    )
    parser.add_argument(
        "--rebalance_eval_cancers",
        action="store_true",
        default=False,
        help="Wether or not to resample cancers in dev/test to match a the expected cancer incidence.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether or not downsample activ",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="Probability mass of test accuracy confidence interval should cover [default: 0.95]",
    )
    parser.add_argument(
        "--num_resamples",
        type=int,
        default=10000,
        help="Number of resamples to use for emperical bootstrap. Should be as large as feasible [default: 1e5]",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["force_num_chan_2d", "normalize_2d"],
        help='List of tensor-transformations to use for the dev and test dataset [default: ["force_num_chan_2d", "normalize_2d"]]\
                        Usage: similar to image_transformers',
    )
    parser.add_argument(
        "--num_workers",
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[-1, -1],
        help='width and height of patch in pixels. [default: "-1,-1" for no patch mode]',
    )
    parser.add_argument(
        "--get_dataset_stats",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="return img feature maps instead of pooled hiddens",
    )
    parser.add_argument(
        "--img_mean",
        type=float,
        nargs="+",
        default=[0.2023],
        help="mean value of img pixels. Per channel. ",
    )
    parser.add_argument(
        "--img_std",
        type=float,
        nargs="+",
        default=[0.2576],
        help="std of img pixels. Per channel. ",
    )
    parser.add_argument(
        "--img_dir",
        type=str,
    ...
    parser.add_argument(
        "--num_chan", type=int, default=3, help="Number of channels in img. [default:3]"
    )
    parser.add_argument(
        "--force_input_dim",
        action="store_true",
        default=False,
        help="trunctate hiddens from file if not == to input_dim",
    )
    parser.add_argument(
        "--input_dim",
        type=int,
        default=512,
        help="Input dim for 2stage models. [default:512]",
    )
    parser.add_argument(
        "--transfomer_hidden_dim",
        type=int,
        default=512,
        help="start hidden dim for transformer",
    )
    parser.add_argument(
        "--num_heads", type=int, default=8, help="Num heads for transformer"
    )
    parser.add_argument(
        "--multi_image",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Simulatenously pred both sides for multi-img model",
    )
    parser.add_argument(
        "--min_num_images",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether the data loaded will be videos (T, H, W, C) or images (H, W, C)",
    )
    parser.add_argument(
        "--metadata_dir", type=str, default=None, help="dir of metadata jsons."
    )
    parser.add_argument(
        "--metadata_path", type=str, default=None, help="path of metadata csv."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If true, drops the samples from a beign breast on an exam that has a malignant one (one datasets that label by side)",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Wether to apply a weighted sampler to balance between the classes on each batch.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--year_weighted_class_bal",
        action="store_true",
        default=False,
        help="Wether to apply a weighted sampler to balance between the classes,year pairs on each batch.",
    )
    parser.add_argument(
        "--device_class_bal",
        action="store_true",
        default=False,
        help="Add device id to dist key for class balancing",
    )
    parser.add_argument(
        "--allowed_devices",
        nargs="*",
        default="all",
        help='List of allowed Devices. None to allow all. Supported are "[Hologic_Selenia, Lorad_Selenia, Selenia_Dimensions, Senograph_DS_ADS_43.10.1]"" ',
    )
    parser.add_argument(
        "--use_c_view_if_available",
        action="store_true",
        default=False,
        help="Wether to to use the C-view if available instead of the regular mammogram.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Wether to add to use a spatial transformer.",
    )
    parser.add_argument(
        "--spatial_transformer_name",
        type=str,
        default="affine",
        help="Type of transformer to use. Can be affine, bounded_tps or unbounded_tps",
    )
    parser.add_argument(
        "--spatial_transformer_img_size",
        nargs="+",
        default=[208, 256],
        help="width and height of image in pixels for STN. Must be <= image_size. May downsample to do this",
    )
    parser.add_argument(
        "--location_network_name",
        type=str,
        default="resnet18",
        help="Model to use for localization network",
    )
    parser.add_argument(
        ...
        help='Layout of blocks for a ResNet model. Must be a list of length 4. Each of the 4 elements is a string of form "block_name,num_repeats-block_name,num_repeats-...". [default: resnet18 layout]',
    )
    parser.add_argument(
        "--tps_grid_size",
        type=int,
        default=10,
        help="Grid size for control points for TPS",
    )
    parser.add_argument(
        "--tps_span_range",
        type=float,
        default=0.9,
        help="Range for relative coords for TPS",
    )
    # regularization
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Wether to add a loss factoring in the collected cancer region annotations .",
    )
    parser.add_argument(
        "--fraction_region_annotation_to_use",
        type=float,
        default=1.0,
        help="Fraction of region annotations to use, i.e 1.0 for all and 0 for none. Used for learning curve analysis.",
    )
    parser.add_argument(
        "--region_annotation_loss_type",
        type=str,
        default="pred_region",
        help='Type of region annotation_loss. Options are ["pred_region", "supervised_attention", "constrastive"]',
    )
    parser.add_argument(
        "--region_annotation_pred_kernel_size",
        type=int,
        default=5,
        help="Used for pred-region region annotation loss. Model should use a k*k kernel when deciding if a pixel is in boudning box or not",
    )
    parser.add_argument(
        "--region_annotation_focal_loss_lambda",
        type=float,
        default=0,
        help="Used for pred-region region annotation loss. Lambda to use for focal loss, where 0 corresponds to regular cross entropy.",
    )
    parser.add_argument(
        "--region_annotation_contrast_alpha",
        type=float,
        default=0.3,
        help="Used for contrastive region annotation loss. Model should alpha more confident given region",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="lambda to weigh the region loss.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help='Wether to add a adversarial loss representing the kl divergernce from source to target domain. Note, dataset obj must provide "target_x" to take effect.',
    )
    parser.add_argument(
        "--use_mmd_adv",
        action="store_true",
        default=False,
        help="Assume use_adv, use MMD instead of parameterized critic",
    )
    parser.add_argument(
        "--add_repulsive_mmd",
        action="store_true",
        default=False,
        help="add loss to max divergence between representations of + -",
    )
    parser.add_argument(
        "--use_temporal_mmd",
        action="store_true",
        default=False,
        help="Assume use_adv and use_mmd_adv, use Temporal MMD formulation with a discounted LIFO cache",
    )
    parser.add_argument(
        "--temporal_mmd_cache_size",
        type=int,
        default=32,
        help="max size of cache for mmd.",
    )
    parser.add_argument(
        "--temporal_mmd_discount_factor",
        type=float,
        default=0.60,
        help="decay factor for hidden age. Weight of vector for mmd is discount ** age.",
    )
    parser.add_argument(
        "--adv_loss_lambda",
        type=float,
        default=0.5,
        help="lambda to weigh the adversarial loss.",
    )
    parser.add_argument(
        "--train_adv_seperate",
        action="store_true",
        default=False,
        help="Alternate between train adv and train model in model steps, every ",
    )
    parser.add_argument(
        "--anneal_adv_loss",
        action="store_true",
        default=False,
        help="Assume use_adv and use_mmd_adv, anneal lambda from 0 to set value in 10000 steps",
    )

    parser.add_argument(
        "--turn_off_model_train",
        action="store_true",
        default=False,
        help="Multiply model loss by 0 so only train adversary.",
    )
    parser.add_argument(
        "--adv_on_logits_alone",
        action="store_true",
        default=False,
        help="Train adversary using only posterior dist.",
    )
    parser.add_argument(
        "--num_model_steps",
        type=int,
        default=1,
        help="num steps of model optimization before switching to adv optimization.",
    )
    parser.add_argument(
        "--num_adv_steps",
        type=int,
        default=100,
        help="max num steps of adv before switch back to model optimization. ",
    )
    parser.add_argument(
        "--wrap_model",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to predict value of all RF from image.",
    )  #
    parser.add_argument(
        "--pred_risk_factors_lambda",
        type=float,
        default=0.25,
        help="lambda to weigh the risk factor prediction.",
    )
    parser.add_argument(
        "--use_pred_risk_factors_at_test",
        action="store_true",
        default=False,
        help="Whether to use predicted risk factor values at test time.",
    )  #
    parser.add_argument(
        "--use_pred_risk_factors_if_unk",
        action="store_true",
        default=False,
        help="Whether to use predicted risk factor values at test time only if rf is unk.",
    )  #
    parser.add_argument(
        "--risk_factor_keys",
        nargs="*",
    ...
    parser.add_argument(
        ...
        help="Path to risk factor metadata file.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to modify model, eval and training for survival analysis.",
    )  #
    parser.add_argument(
        "--make_probs_indep",
        action="store_true",
        default=False,
        help="Make surival model produce indepedent probablities.",
    )  #
    parser.add_argument(
        "--mask_mechanism",
        default="default",
        help="How to mask for survival objective. options [default, indep, slice, linear].",
    )  #
    parser.add_argument(
        "--eval_survival_on_risk",
        action="store_true",
        default=False,
        help="Port over survival model to risk model.",
    )  #
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Port over risk model to survival model.",
    )  #
    # generative modeling
    parser.add_argument(
        "--mask_prob",
        type=float,
        default=0,
        help="Amount of masking to apply on images",
    )
    parser.add_argument(
        "--pred_missing_mammos",
        action="store_true",
        default=False,
        help="Whether to predict missing images when doing image dropout.",
    )  #
    parser.add_argument(
        "--also_pred_given_mammos",
        action="store_true",
        default=False,
        help="Whether to predict given images.",
    )  #
    parser.add_argument(
        "--pred_missing_mammos_lambda",
        type=float,
        default=0.25,
        help="lambda to weigh the pred_missing_mammos.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Whether to only use hiddens from a pretrained model.",
    )
    parser.add_argument(
        "--zero_out_hiddens",
        action="store_true",
        default=False,
        help="Zero hiddens from image to test bias",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to results file with hiddens for the whole dataset.",
    )
    parser.add_argument(
        "--use_dev_to_train_model_on_hiddens",
        action="store_true",
        default=False,
        help="Whether to use first half of dev to train model (assuming use_precomputed_hiddens)",
    )
    parser.add_argument(
        "--turn_off_init_projection",
        action="store_true",
        default=False,
        help="Whether to not project base hiddens before use",
    )
    ...
    parser.add_argument(
        ...
        help="objective function to use [default: cross_entropy]",
    )
    parser.add_argument(
        "--init_lr",
        type=float,
    ...
    parser.add_argument(
        "--turn_off_model_reset",
        action="store_true",
        default=False,
        help="Don't reload the model to last best when reducing learning rate",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Splits batch size into smaller batches in order to fit gpu memmory limits. Optimizer step is run only after one batch size is over. Note: batch_size/batch_splits should be int [default: 1]",
    )
    parser.add_argument(
        "--dropout",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Train without tuning on dev (no adaptive lr reduction or saving best model based on dev)",
    )
    parser.add_argument(
        "--lr_reduction_interval",
        type=int,
        default=1,
        help="Number of epochs to wait before reducing lr when training without adaptive lr reduction.",
    )
    parser.add_argument(
        "--data_fraction",
        type=float,
        default=1.0,
        help="Fraction of data to use, i.e 1.0 for all and 0 for none. Used for learning curve analysis.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If true, use 10-fold cross validation.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Index of the partition to hold out as test.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Num layers for transformer based models.",
    )
    parser.add_argument(
        "--snapshot",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="filename of img_feat_extractor model snapshot to load. Only used for mirai_full type models [default: None]",
    )
    parser.add_argument(
        "--freeze_image_encoder",
        action="store_true",
        default=False,
        help="Whether freeze image_encoder.",
    )  #
    parser.add_argument(
        "--transformer_snapshot",
        type=str,
        default=None,
        help="filename of transformer model snapshot to load. Only used for mirai_full type models [default: None]",
    )
    parser.add_argument(
        "--calibrator_snapshot",
        type=str,
        default=None,
        help="filename of calibrator. Produced for a single model on development set using Platt Scaling",
    )
    parser.add_argument(
        "--patch_snapshot",
        type=str,
        default=None,
        help="filename of patch model snapshot to load. Only used for aggregator type models [default: None]",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Replace last linear layer with convolutional layer",
    )
    parser.add_argument(
        "--replace_bn_with_gn",
        action="store_true",
        default=False,
        help="Use group normalization instead of batch norm.",
    )
    ...
    parser.add_argument(
        "--block_widening_factor",
        type=int,
        default=1,
        help="Factor by which to widen blocks.",
    )
    parser.add_argument(
        "--num_groups",
        type=int,
        default=1,
        help="Num groups per conv in Resnet blocks.",
    )
    parser.add_argument(
        "--pool_name", type=str, default="GlobalAvgPool", help="Pooling mechanism"
    )
    parser.add_argument(
        "--deep_risk_factor_pool",
        action="store_true",
        default=False,
        help="make risk factor pool use several layers to fuse image and rf info",
    )
    parser.add_argument(
        "--replace_snapshot_pool",
        action="store_true",
        default=False,
        help="Use detached models",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Change all paths accordingly.",
    )
    parser.add_argument(
        "--cuda", action="store_true", default=False, help="enable the gpu"
    )
    parser.add_argument(
        "--num_gpus", type=int, default=1, help="Num GPUs to use in data_parallel."
    )
    parser.add_argument(
        "--num_shards", type=int, default=1, help="Num GPUs to shard a single model."
    )
    parser.add_argument(
        "--data_parallel",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Produce plots of losses/acc",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Calculate accuracy of predictions by exam instead of by image",
    )
    parser.add_argument(
        "--background_size",
        type=int,
        nargs="+",
        default=[1024, 1024],
        help="The size of the background for the mnist_background dataset",
    )
    parser.add_argument(
        "--noise",
        action="store_true",
        default=False,
        help="Whether to add noise to images in the mnist_background dataset",
    )
    parser.add_argument(
        "--noise_var",
        type=float,
        default=0.1,
        help="Variance of gaussian noise added to mnist_background dataset",
    )
    parser.add_argument(
        "--use_permissive_cohort",
        action="store_true",
        default=True,
        help="Allow exams with cancer within 1 year before and after.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="type of mammograms used to evaluate model",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="whether to resume if run has already been done",
    )
    parser.add_argument(
        "--ignore_warnings",
        action="store_true",
...
def validate_args(args):
    ...
    if args.batch_size % args.batch_splits != 0:
        raise ValueError(
            BATCH_SIZE_SPLIT_ERR.format(args.batch_size, args.batch_splits)
        )
