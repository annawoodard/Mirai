...
def dicom_to_image_dcmtk(dicom_path, image_path):
    ...
    if "GE" in manufacturer and voi_lut_exists:
        ...
        logger.debug(
            "SeriesDescription contains C-View and VOI LUT exists, using VOI LUT"
        )
        args = [
            "dcmj2pnm",
            "+on2",
...
def dicom_to_arr(dicom, window_method="minmax", index=0, pillow=False, overlay=False):
    ...
    if "GE" in manufacturer and (0x0028, 0x3010) in dicom:
        ...
        logger.debug(
            f"auto window center: {window_center}, window width: {window_width}"
        )
        ...
        logger.debug(
            f"minmax window center: {window_center}, window width: {window_width}"
        )
