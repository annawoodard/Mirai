...
RESAMPLE_FAILED_WARNING = (
    "Resampling distrubution for estimator {} failed because of : {}"
)
...
def confidence_interval(
    ...
    while len(deltas) < num_resamples:
        if num_tries > num_resamples * 10:
            raise Exception(
                CONFIDENCE_INTERVAL_EXCEPTION.format(
                    num_tries, len(deltas), num_resamples
                )
            )
        num_tries += 1
        try:
            if clusters is None:
                ...
                resample_clusters = np.random.choice(
                    a=cluster_ids, size=num_clusters, replace=True
                )
                resample_ind = []
    ...
    lower_bound, upper_bound = ci_bounds(
        emp_mean, deltas, confidence_interval, num_resamples
    )
...
def resample_set_by_distribution(sets, probs, length):
    ...
    idx = np.random.choice(
        np.arange(len(concat_array)), size=length, replace=True, p=probs_array
    )
    return concat_array[idx], set_inds[idx]


def get_rebalanced_cancer_set(
    probs, golds, human_preds=None, ratio=0.59, rebalance_eval_cancers=False
):
    """
    ...
    if rebalance_eval_cancers:
        ...
        resampled_probs, resampled_labels = resample_set_by_distribution(
            [probs[benign_idxs], probs[cancer_idxs]], sample_probs, len(probs)
        )
        assert human_preds is None
    else:
        resample_ind = np.random.choice(
            a=range(len(probs)), size=len(probs), replace=True
        )
...
def get_thresholds_interval(
    ...
    for _ in tqdm.tqdm(range(num_resamples)):
        resampled_probs, resampled_labels, resampled_human_preds = (
            get_rebalanced_cancer_set(
                probs,
                golds,
                human_preds=human_preds,
                rebalance_eval_cancers=rebalance_eval_cancers,
            )
        )
        pos_probs = resampled_probs[np.where(resampled_labels == 1)]
        human_negs = resampled_human_preds < 0.5
        human_pos = resampled_human_preds > 0.5
    ...
    lower_bound, upper_bound = ci_bounds(
        th_mean, deltas, confidence_interval, num_resamples
    )
...
def get_rates_intervals(
    ...
    tnrs = []
    for _ in tqdm.tqdm(range(num_resamples)):
        resampled_probs, resampled_labels = get_rebalanced_cancer_set(
            probs, golds, rebalance_eval_cancers=rebalance_eval_cancers
        )
        fnr, tpr, tnr = get_rates_by_threshold(
            resampled_probs, resampled_labels, threshold
        )
        fnrs.append(fnr)
        tprs.append(tpr)
        tnrs.append(tnr)
    ...
    def mean_and_bounds(a):
        ...
        lower_bound, upper_bound = ci_bounds(
            mean, deltas, confidence_interval, num_resamples
        )
