...
class RiskFactorVectorizer:
    def __init__(self, args):
        ...
        self.risk_factor_transformers = {
            ...
            "ashkenazi": self.get_binary_transformer(
                "ashkenazi", use_patient_factors=True
            ),
            "brca": self.transform_brca,
            "mom_bc_cancer_history": self.get_binary_relative_cancer_history_transformer(
                "M"
            ),
            "m_aunt_bc_cancer_history": self.get_binary_relative_cancer_history_transformer(
                "MA"
            ),
            "p_aunt_bc_cancer_history": self.get_binary_relative_cancer_history_transformer(
                "PA"
            ),
            ...
            "brother_bc_cancer_history": self.get_binary_relative_cancer_history_transformer(
                "B"
            ),
            "father_bc_cancer_history": self.get_binary_relative_cancer_history_transformer(
                "F"
            ),
            "daughter_bc_cancer_history": self.get_binary_relative_cancer_history_transformer(
                "D"
            ),
            "sister_bc_cancer_history": self.get_binary_relative_cancer_history_transformer(
                "S"
            ),
            ...
            "hrt_years_ago_stopped": self.get_hrt_information_transformer(
                "years_ago_stopped"
            ),
        }
        ...
        for k in self.risk_factor_keys:
            ...
            names = self.risk_factor_transformers[k](
                None, None, just_return_feature_names=True
            )
    ...
    def one_hot_feature_names(self, risk_factor_name, cutoffs):
        ...
        for i in range(1, len(cutoffs)):
            feature_names[i] = "{}_{}_{}".format(
                risk_factor_name, cutoffs[i - 1], cutoffs[i]
            )
    ...
    def get_binary_occurence_transformer(self, occurence_key, occurence_age_key):
        def transform_binary_occurence(
            ...
            exam_age = int(exam_factors["age"])
            if occurence and (
                occurence_age == MISSING_VALUE or exam_age >= occurence_age
            ):
    ...
    def get_binary_relative_cancer_history_transformer(
        self, relative_code, cancer="breast_cancer"
    ):
    ...
    def transform_parous(
        ...
        if patient_factors["first_pregnancy_age"] != MISSING_VALUE:
            binary_parous[0] = (
                1 if patient_factors["first_pregnancy_age"] < exam_age else 0
            )
    ...
    def get_hrt_information_transformer(self, piece):
        def transform_hrt_information(
                ...
                if last_age >= current_age and current_age != MISSING_VALUE:
                    if first_age != MISSING_VALUE and first_age > current_age:
                        ...
                        duration = (
                            ...
                            if current_age != MISSING_VALUE
                            and first_age != MISSING_VALUE
            ...
            piece_to_feature_names = {
                ...
                "years_ago_stopped": self.one_hot_vectorizor(
                    hrt_years_ago_stopped, year_cutoffs
                ),
            }
            return piece_to_feature_names[piece]
    ...
    def get_risk_factors_for_sample(self, sample):
        ...
        sample_exam_factors = self.risk_factor_metadata[sample["ssn"]]["accessions"][
            sample["exam"]
        ]
    ...
    def get_buckets_for_sample(self, sample):
        ...
        buckets = {}
        for key in self.risk_factor_keys:
            names = self.risk_factor_transformers[key](
                None, None, just_return_feature_names=True
            )
            vectorized = self.risk_factor_transformers[key](
                sample_patient_factors, sample_exam_factors
            )
            if sum(vectorized) == 0:
...
def parse_risk_factors(args):
    ...
    if "prior_hist" in args.risk_factor_keys:
        for patient in metadata_json:
            if "nwh" in args.dataset:
                ...
                for exam in patient["accessions"]:
                    acc = exam["accession"]
                    risk_factor_metadata[ssn]["accessions"][acc]["prior_hist"] = exam[
                        "prior_hist"
                    ]
    if "years_to_cancer" in args.risk_factor_keys:
        for patient in metadata_json:
            ...
            for exam in patient["accessions"]:
                acc = exam["accession"]
                risk_factor_metadata[ssn]["accessions"][acc]["years_to_cancer"] = exam[
                    "years_to_cancer"
                ]
    if "bpe" in args.risk_factor_keys:
        for patient in metadata_json:
            ...
            for exam in patient["accessions"]:
                acc = exam["accession"]
                risk_factor_metadata[ssn]["accessions"][acc]["bpe"] = (
                    exam["bpe"] if "bpe" in exam else MISSING_VALUE
                )
