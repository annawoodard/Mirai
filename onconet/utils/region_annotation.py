...
REGION_ANNOTATION_FILE_NOTFOUND_ERR = (
    "Region annotation file {} could not be parsed! Exception: {}!"
)
...
def get_annotation_mask(x, batch, volatile, args):
    ...
    _left_x = (
        torch.min(batch["region_bottom_left_x"], batch["region_top_left_x"])
        .unsqueeze(1)
        .unsqueeze(-1)
        .unsqueeze(-1)
        .float()
    ...
    _right_x = (
        torch.max(batch["region_bottom_right_x"], batch["region_top_right_x"])
        .unsqueeze(1)
        .unsqueeze(-1)
        .unsqueeze(-1)
        .float()
    ...
    _top_y = (
        torch.min(batch["region_top_left_y"], batch["region_top_right_y"])
        .unsqueeze(1)
        .unsqueeze(-1)
        .unsqueeze(-1)
        .float()
    ...
    _bottom_y = (
        torch.max(batch["region_bottom_left_y"], batch["region_bottom_right_y"])
        .unsqueeze(1)
        .unsqueeze(-1)
        .unsqueeze(-1)
        .float()
    ...
    w_mask = ((w_arr + half_w_step) >= left_x).long() & (
        (w_arr - half_w_step) <= right_x
    ).long()
    h_mask = ((h_arr + half_h_step) >= top_y).long() & (
        (h_arr - half_h_step) <= bottom_y
    ).long()
...
def parse_region_annotations(args):
    ...
    try:
        ...
        raise Exception(
            REGION_ANNOTATION_FILE_NOTFOUND_ERR.format(REGION_ANNOTATION_FILE, e)
        )
...
def get_region_annotation_for_sample(sample, region_annotations, args):
    if args.multi_image:
        return [
            get_region_annotation_for_path(path, region_annotations, args, indx)
            for indx, path in enumerate(sample["paths"])
        ]
    else:
    ...
    if path in region_annotations:
        ...
        if len(annotations) > 0:
            ...
            if "align_to_left" in args.image_transformers and IMAGE_RIGHT_ALIGNED[path]:
                ...
                region_annotation = flip_region_coords_left_right(
...
def rotate_region_coords_angle(angle, additional):
    if additional is not None and "region_annotation" in additional:
        ...
        point_keys = [
            "region_bottom_left_{}",
            "region_bottom_right_{}",
            "region_top_left_{}",
            "region_top_right_{}",
        ]

        points = [
            np.array([region[key.format("x")], region[key.format("y")]])
