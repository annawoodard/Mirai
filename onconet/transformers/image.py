...
from onconet.utils.region_annotation import (
    flip_region_coords_left_right,
    flip_region_coords_top_bottom,
    rotate_region_coords_angle,
    make_region_annotation_blank,
)
import warnings
...
def in_overlays(x1, y1, patch_width, patch_height, overlays, scaler, img_origin_size):
    for overlay in overlays:
        if in_overlay(
            x1, y1, patch_width, patch_height, overlay, scaler, img_origin_size
        ):
            return True
...
def in_overlay(x1, y1, patch_width, patch_height, overlay, scaler, img_origin_size):
    patch = Rectangle(Point(x1, y1 + patch_height), Point(x1 + patch_width, y1))
    x, y = scaler.get_xy(
        img_origin_size[0],
        img_origin_size[1],
        overlay["boundary"]["min_x"],
...
class ExtractPatch(Abstract_transformer):
    ...
    def __init__(self, args, kwargs):
        ...
        self.patch_scaler = torchvision.transforms.Resize(
            (self.patch_height, self.patch_width)
        )
        self.random_cropper = torchvision.transforms.RandomCrop(
            (self.patch_width, self.patch_height)
        )

    def __call__(self, img, additional=None):
        label = additional["label"]
        zoom_factor = np.random.uniform(low=1 - self.zoom, high=1 + self.zoom)
        ...
        if label == 0:
            ...
            while patch_in_overlay:
                ...
                patch_in_overlay = in_overlays(
                    ...
                    patch_width,
                    patch_height,
                    additional["all_overlays"],
                    self.cord_rescaler,
                    (additional["width"], additional["height"]),
                )

            return self.patch_scaler(
                img.crop(
                    (
                        x1 - patch_width // 2,
                        y1 - patch_width // 2,
            ...
            x1, y1 = self.cord_rescaler.get_xy(
                additional["width"],
                additional["height"],
...
class Random_Crop(Abstract_transformer):
    ...
    def __call__(self, img, additional=None):
        if (
            self.args.use_region_annotation
            and additional is not None
            and "region_annotation" in additional
        ):
...
class Rotate_90(Abstract_transformer):
    ...
    def __init__(self, args, kwargs):
        ...
        assert len(kwargs.keys()) == 0
        self.args = args
        self.rotations = [0, Image.ROTATE_90, Image.ROTATE_180, Image.ROTATE_270]

        self.rotation_to_angle = {
            0: 0,
            Image.ROTATE_90: 90,
            Image.ROTATE_180: 180,
            Image.ROTATE_270: 270,
        }
