...
class TestModels(unittest.TestCase):
    ...
    def test_calibrator_regression(self):
        ...
        """
        new_cal_path = os.path.join(
            project_dir, "snapshots/calibrators/Mirai_calibrator_mar12_2022.p"
        )
        cal_table_path = os.path.join(
            project_dir,
            "tests/test_data/Mirai_pred_rf_callibrator_mar12_2022_values.csv",
        )
        ...
        for key, calibrator in new_calibrators.items():
            ...
            atol = 1e-12
            assert np.allclose(new_vals, exp_vals, atol=atol), (
                f"key: {key}, old: {exp_vals}, new: {new_vals}"
            )
...
def _generate_calibration_data():
    ...
    old_cal_path = (
        "snapshots_v0.7.0/callibrators/Mirai_pred_rf_callibrator_mar12_2022.p"
    )
    import pickle
