...
ADDITIONAL_RESULT_KEYS = [
    "dev_auc",
    "dev_tnr_by_threshold",
    "dev_fnr_by_threshold",
    "test_fnr_by_threshold",
    "test_tnr_by_threshold",
]
...
parser = argparse.ArgumentParser(
    description="OncoNet Grid Search Dispatcher. For use information, see `doc/README.md`"
)
parser.add_argument(
    "--experiment_config_path",
    required=True,
    type=str,
    help="Path of experiment config",
)
parser.add_argument(
    "--log_dir",
    type=str,
...
parser.add_argument(
    "--result_path",
    type=str,
    default="results/grid_search.csv",
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="whether to rerun experiments with the same result file location",
)
parser.add_argument(
    "--shuffle_experiment_order",
    action="store_true",
    default=False,
    help="whether to shuffle order of experiments",
)
...
def launch_experiment(gpu, flag_string):
    ...
    experiment_string = (
        "CUDA_VISIBLE_DEVICES={} python -u scripts/main.py {} --results_path {}".format(
...
def update_sumary_with_results(result_path, log_path, experiment_axies, summary):
    ...
    best_epoch_indx = (
        result_dict["epoch_stats"]["best_epoch"] if result_dict["train"] else 0
    )
    present_result_keys = []
    for k in result_keys:
        if (
            (
                "test_stats" in result_dict
                and k in result_dict["test_stats"]
                and len(result_dict["test_stats"][k]) > 0
            )
            or (
                "dev_stats" in result_dict
                and k in result_dict["dev_stats"]
                and len(result_dict["dev_stats"][k]) > 0
            )
            or (
                result_dict["train"]
                and k in result_dict["epoch_stats"]
                and len(result_dict["epoch_stats"][k]) > 0
            )
        ):
    ...
    for k in ADDITIONAL_RESULT_KEYS:
        ...
        if stats_key in result_dict and k in result_dict[stats_key]:
            if k not in present_result_keys:
                present_result_keys.append(k)
    ...
    for prev_summary in summary:
        if len(set(prev_summary.keys()).union(set(summary_columns))) > len(
            summary_columns
        ):
...
if __name__ == "__main__":
    ...
    for gpu in experiment_config["available_gpus"]:
        ...
        multiprocessing.Process(
            target=worker, args=(gpu, job_queue, done_queue)
        ).start()
    print()
    ...
    for i in range(len(job_list)):
        ...
        summary = update_sumary_with_results(
            result_path, log_path, experiment_axies, summary
        )
